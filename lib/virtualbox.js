...
var metrics = {
  setup: function(options, callback) {
    var vm = options.vm || options.name || options.vmname || options.title,
        ...
        metrics = options.metrics || "CPU/Load/User,CPU/Load/Kernel,Net/Rate/Rx,Net/Rate/Tx";


    var cmd = `metrics setup --period ${period} --samples ${samples} "${vm}" "${metrics}"`;
    vboxmanage(cmd, function(error, stdout) {
      if (error) {
        callback(error);
        return;
  ...
  query: function(options, callback) {
    ...
    var cmd = `metrics query "${vm}" "${metrics}"`;
    vboxmanage(cmd, function(error, stdout) {
      if (error) {
        callback(error);
        return;
